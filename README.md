...
基于 **Spring AI + OpenAI** 的智能 Pull Request 代码审查助手。传入 GitHub PR 链接，自动获取变更内容并调用大模型进行多维度代码审查，输出结构化的 Review 报告。
...
- OpenAI API Key（或兼容的 API Endpoint）
...
也可在 `src/main/resources/application.yml` 中直接修改占位符（仅限本地开发）。
...
| 模型能力 | GPT-4o | 代码理解能力在主流模型中名列前茅，支持 128K 上下文窗口，足以承载典型 PR 的 Diff 量 |
...
每一层职责清晰：Controller 负责参数校验和路由，Service 封装外部调用和 AI 交互逻辑，DTO 定义数据契约。层与层之间通过接口而非具体实现通信，便于单元测试和后续替换。
...
1. **Token 限制**：GPT-4o 的上下文窗口为 128K tokens，但一次性消耗过多 tokens 会挤压输出空间，且费用高昂。
...
替代当前的简单硬截断，引入 AST 解析（如 `javaparser`）识别代码块的语义边界，按方法/类为单位进行智能分片。每个分片独立送审，结果汇总时按风险严重程度排序，确保不因截断遗漏关键变更。
